...
def opt_callgraph(args, binary):
    ...
    prog = PROJ_ROOT / "SVF/Release-build/bin/wpa"
    cmd = [prog, "--dump-callgraph", "--ander", f"{binary}"]    
    log_p = args.temporary_directory / f"step{STEP}.log"
    with log_p.open("w") as f:
        try:
            subprocess.run(cmd, stderr=f, check=True, cwd=dot_files)
            os.rename(dot_files / f"callgraph_final.dot", dot_files / f"{binary.name}.callgraph.dot")
            cmd = ["sed", "-i", "s/:s[0-9]*//g", dot_files / f"{binary.name}.callgraph.dot"]   
            subprocess.run(cmd, stderr=f, check=True, cwd=dot_files)
            regex="s/{fun: main/{fun: %s:main/g"%(str(args.fuzzer_name).split('/')[-1])
            cmd = ["sed", "-i", regex, dot_files / f"{binary.name}.callgraph.dot"]   
            subprocess.run(cmd, stderr=f, check=True, cwd=dot_files)
...
def exec_distance_prog(dot, targets, out, names, cg_distance=None,
                       cg_callsites=None, cg_incallsites=None, f_index=None, bb_index=None, f_out_index=None,
                       bb_icallmap=None, py_version=False):
    """
    Args:
...
def calculating_distances(args):
    ...
    bbincalls = args.temporary_directory / "BBinCalls.txt"
    ...
    findex = args.temporary_directory / "F_index.txt"
    foutindex = args.temporary_directory / "F_out_index.txt"
    bbindex = args.temporary_directory / "BB_index.txt"
    bbicall = args.temporary_directory / "BBicalls.txt"
    ...
    total = len(os.listdir(dot_files)) - 2
    # Helper
    ...
    with ThreadPoolExecutor(max_workers=mp.cpu_count()) as executor:
        ...
        for i in results:
            count += 1
            print("\r", end="")
            print("now: {}%: ".format(int(count * 100/total)), "*" * (int(count * 100/total) // 2), end="")
            sys.stdout.flush()
